...
// SMTPTarget represents a destination to which mail may be sent
...
// New creates a new SMTPTarget to which you can send mail. server should be
// a host and port in the format "smtp.example.com:587". Username and
// password are required. Anonymous authentication is not supported. timeout
// is how long the connection will be allowed to sit idle before it is
// disconnected. It will automatically re-connect next time you call
// SendMail().
func New(server, username, password string, timeout time.Duration) SMTPTarget {
	t := SMTPTarget{
		server:          server,
		username:        username,
		password:        password,
...
// watcher thread. The SMTPTarget cannot be used after this. You must make
// a new one.
func (t SMTPTarget) Close() error {
	...
	if *t.closed {
		return errors.New("SMTPTarget is already closed")
	}
...
// from the SMTPTarget. It will first re-establish the connection if the
// connection is unhealthy or not connected.
func (t SMTPTarget) SendMail(from string, to []string, msg []byte) error {
	...
	if *t.closed {
		return errors.New("SMTPTarget is closed")
	}
...
func (t SMTPTarget) reset() (err error) {
	switch {
	...
	case t.client == nil:
		...
		t.client, err = smtp.Dial(t.server)
		if err != nil {
			return err
		}
		...
		err = t.client.Hello(localName)
		if err != nil {
			return err
		}

		// start TLS (encryption)
		serverName, _, err := net.SplitHostPort(t.server)
		if err != nil {
			return err
		}
		err = t.client.StartTLS(&tls.Config{ServerName: serverName})
		if err != nil {
			return err
		}

		// authenticate
		auth := smtp.PlainAuth("", t.username, t.password, serverName)
...
// NOTE: this is NOT threadsafe
func (t SMTPTarget) disconnect() error {
	if t.client == nil {
		return nil
	}
	err := t.client.Quit()
